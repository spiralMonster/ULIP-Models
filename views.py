...
from ULIP.Models.prophet_model import ProphetModel
from ULIP.Models.lstm_cnn_hybrid_model import LSTMAndCNN4StockForecasting
import joblib
import pandas as pd
import numpy as np
import json
...
from RouteOptimization.RoutePlanning.route_planning import PerformRoutePlanning
...
def get_routes(request,*args,**kwargs):
    if request.method=='POST':
        try:
            ...
            # time=data.get('time')
            # carbon=data.get('carbonEmission')
            results=PerformRoutePlanning(model,source,destination)
            # results['totalDistanceCovered']=distance
            # results['totalTimeRequired']=time
            # results['totalCarbonEmitted']=carbon
            print(results)
            try:
...
def get_stock_forecasting(request,*args,**kwargs):
    if request.method=='POST':
        try:
            ...
            data=pd.read_csv('/home/amartya/Dev-Pycharm Projects/django/ULIP/ULIP/ulip_store_final_dataset.csv')
            data=data[data['Product Name'].isin(products)]
